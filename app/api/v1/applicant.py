...
async def get_applicants_by_job_posting_endpoint(
    ...
    result = await get_applicants_by_job_posting(job_posting_id)
    logger.info(
...
async def get_all_applicants_by_corporate_user_endpoint(
    ...
    result = await get_applicants_by_corporate_user(current_user.id)
...
async def get_seeker_applications_endpoint(
    ...
    applications = await get_applicants_by_seeker_user(current_user.id)
...
async def get_applicant_detail_endpoint(
    ...
    result = await get_applicant_by_id(applicant_id, current_user.id)
    if result is None:
