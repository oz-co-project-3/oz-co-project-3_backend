...
@job_posting_router.post(
    ...
    description=(
        ...
        "`404` `code`: `user_not_found` - 해당 유저를 찾을 수 없습니다.\n"
...
async def create_job_posting_endpoint(
    ...
    result = await create_job_posting(current_user, data)
...
async def get_my_company_job_postings(
    ...
    result = await get_job_postings_by_company_user(corporate_user)
...
async def get_specific_job_posting_endpoint(
    ...
    corporate_user = await get_corporate_user_by_base_user(current_user)
    job_posting = await get_specific_job_posting(corporate_user, job_posting_id)
    logger.info(
...
async def patch_job_posting_endpoint(
    ...
    corporate_user = await get_corporate_user_by_base_user(current_user)
    result = await patch_job_posting(corporate_user, job_posting_id, updated_data)
    logger.info(
...
async def delete_job_posting_endpoint(
    ...
    corporate_user = await get_corporate_user_by_base_user(current_user)
    await delete_job_posting(corporate_user, job_posting_id)
    logger.info(
...
async def create_job_posting_bookmark(
    ...
    is_added = await toggle_bookmark(current_user, id)
