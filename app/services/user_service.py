...
from app.schemas.user_schema import CompanyRegisterRequest, CompanyRegisterResponse, CompanyRegisterResponseData, UserRegisterRequest, UserRegisterResponse, UserRegisterResponseData
...
async def register_user(request: UserRegisterRequest) -> UserRegisterResponse:
    ...
    if existing_user:
        raise CustomException(status_code=400, error="중복된 이메일입니다.", code="duplicate_email")

    if len(request.password) < 8 or not re.search(r"[!@#$%^&*(),.?\":{}|<>]", request.password):
        raise CustomException(status_code=400, error="비밀번호는 최소 8자 이상이며 특수 문자를 포함해야 합니다.", code="invalid_password")

    if request.password != request.password_check:
        raise CustomException(status_code=400, error="비밀번호와 비밀번호 확인이 일치하지 않습니다.", code="password_mismatch")
...
async def register_company_user(request: CompanyRegisterRequest) -> CompanyRegisterResponse:
    existing_user = await BaseUser.get_or_none(email=request.email)
    if existing_user:
        raise CustomException(status_code=400, error="중복된 이메일입니다.", code="duplicate_email")
