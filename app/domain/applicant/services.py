...
from app.domain.applicant.repository import (
    get_applicant_by_id,
    get_applicants_by_corporate_user,
...
from app.domain.job_posting.repository import get_job_posting_by_id
...
async def get_applicants_by_job_posting(
    ...
    await validate_user_permissions(user, job_posting)
...
async def get_all_applicants_by_corporate_user(
    current_user: BaseUser, get_applicants_by_corporate_user=None
) -> List[ApplicantResponse]:
    ...
    await validate_user_permissions(corporate_user, corporate_user)

    applicants = await get_applicants_by_corporate_user(corporate_user.id)
    if not applicants:
        logger.warning(
...
async def get_applicants_by_seeker_user(user: BaseUser) -> List[ApplicantResponse]:
    applicants = await get_applicants_by_seeker_user(user.id)
    if not applicants:
        logger.warning(
...
async def get_applicant_detail(user: BaseUser, applicant_id: int) -> ApplicantResponse:
    applicant = await get_applicant_by_id(applicant_id, user.id)
    if not applicant:
        logger.warning(
